...
- **`mobile-porter`** (`.claude/agents/mobile-porter.md`) — makes each **newly added game** playable on a phone. SPEC 10 (`specs/10-movil-tactil.md`) built the shared touch machinery (`touch-controls.tsx`, the `TouchButton` types, the on-screen console CSS, canvas scaling, orientation notice) and applied it to the 4 games of the time; `/add-game` does **not** include a touch step, so a new game ships without a gamepad. This agent extends SPEC 10 to games that arrive later: it demands a target game, reads how that game's component handles the keyboard (polls `keys[code]` per frame → `hold`; acts on `keydown` → `tap`/`repeat`), designs the button map with evidence, adds `touchControls` to its `GAME_REGISTRY` entry — usually the only change — and playtests it at 390×844 / 360×800 (no stuck keys, canvas scales, portrait notice, save-score flow, desktop unaffected). It never edits game components (touch works by synthesizing `KeyboardEvent`s), never touches Supabase, specs, or the rest of the site, and Tito commits each game himself. Invoke it right after `/add-game`.
- **`game-performance-booster`** (`.claude/agents/game-performance-booster.md`) — audits and fixes the **performance of one game**, given its `id`. SPEC 12 (`specs/12-rendimiento-ranaria.md`, Implemented) diagnosed and fixed the costs that made `ranaria` stutter, and `ranaria-game.tsx` is the reference implementation (`SPRITE_PAD`/`makeSprite`/`stamp`, `RenderCache`, `FpsMeter`). This agent generalizes it to any game: it demands a target, audits the six known antipatterns with `file:line` evidence (`shadowBlur` per entity inside the frame loop, static background redrawn every frame, continuous `onExtraStatChange` re-rendering `GamePlayer` 10×/s, drawing while paused, `getContext("2d")` with alpha, caches not invalidated by skin), **measures before optimizing** (in-canvas `?fps=1` meter + A/B canvas-command counting, since Playwright's vsync caps FPS), then fixes them in one file only — `app/components/games/<id>-game.tsx`. It never touches `game-player.tsx`, `globals.css`, `registry.ts`, Supabase or specs, never changes gameplay, and never lowers `pal.glow` (that's `skin-designer`'s turf); the three skins must look pixel-identical after. Tito commits each block himself.
